...
static int my_getattr(const char *path, struct stat *stbuf)
{
    ...
    if(strcmp(path,"/.Trash") == 0){
    	stbuf->st_mode = S_IFDIR | 0700; //¿  S_ISVTX ?
    	stbuf->st_nlink = 2;
    	stbuf->st_uid = getuid();
    	stbuf->st_gid = getgid();
    	stbuf->st_mtime = stbuf->st_ctime = myFileSystem.superBlock.creationTime;
...
static int my_mknod(const char *path, mode_t mode, dev_t device)
{
    ...
    myFileSystem.nodes[idxNodoI]->tipo = 0;
...
static int my_unlink(const char* path)
{
	int idxNode = 0;
		...
		idxNode = findFileByName(&myFileSystem, (char*) path + 1);
		...
		//Rellenamos el campo libre del inodo a true
...
static int my_mkdir(const char *path, mode_t mode) {
	...
	myFileSystem.nodes[idxNodoI]->fileSize = 0;
	myFileSystem.nodes[idxNodoI]->numBlocks = 0;
	myFileSystem.nodes[idxNodoI]->tipo = 1;
	myFileSystem.nodes[idxNodoI]->modificationTime = time(NULL);
	myFileSystem.nodes[idxNodoI]->freeNode = false;
...
struct fuse_operations myFS_operations = {
    .getattr	= my_getattr,					// Obtain attributes from a file -Obtener atributos de un fichero
    .readdir	= my_readdir,					// Read directory entries - Leer entradas del directorio
    .truncate	= my_truncate,					// Modify the size of a file - Modificar el tamaño de un fichero
    .open		= my_open,						// Oeen a file - Abrir un fichero
    .write		= my_write,						// Write data into a file already opened -Escribir datos en un fichero abierto
    .release	= my_release,					// Close an opened file - Cerrar un fichero abierto
    .mknod		= my_mknod,						// Create a new file - Crear un fichero nuevo
	.read		= my_read,						//lee un fichero abierto y devulve la cantidad de bytes leidos
	.unlink		= my_unlink,					//elimina un fichero
	.mkdir		= my_mkdir,						//crea un directorio con un nombre y una direccion
