...
{
	if (lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * BITMAP_IDX, SEEK_SET) == (off_t) - 1){ //posiciono puntero en virtualdisk para leer bitmap B1
		perror("Failed lseek in Bitmap");
		return -1;
	}
	if(read(myFileSystem->fdVirtualDisk, myFileSystem->bitMap , sizeof(BIT) * NUM_BITS) == -1){
		perror("Failed read in Bitmap");
		return -1;
	}
    return 0;
}
...
int readInodes(MyFileSystem* myFileSystem)
{
	...
	for(i = 0; i < MAX_NODES; i++){
		posNode = findNodeByPos(i);
	   	lseek(myFileSystem->fdVirtualDisk,posNode, SEEK_SET);
	   	read(myFileSystem->fdVirtualDisk, &nodeI, sizeof(NodeStruct));

	    if(!nodeI.freeNode){
	    	myFileSystem->numFreeNodes--;
	    	myFileSystem->nodes[i] = malloc(sizeof(NodeStruct));
	    	copyNode(myFileSystem->nodes[i], &nodeI);
	    }
	}
    return 0;
}
...
int myMount(MyFileSystem *myFileSystem, char *backupFileName)
{
	...
	myFileSystem->fdVirtualDisk = open(backupFileName, O_RDWR);



	lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * BITMAP_IDX, SEEK_SET);
	read(myFileSystem->fdVirtualDisk, myFileSystem->bitMap , sizeof(BIT) * NUM_BITS);

	lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * SUPERBLOCK_IDX, SEEK_SET);
	read(myFileSystem->fdVirtualDisk, &myFileSystem->superBlock, sizeof(SuperBlockStruct));
	...
	int posNode = 0;
		...
		lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * DIRECTORY_IDX, SEEK_SET);
		read(myFileSystem->fdVirtualDisk, &(myFileSystem->directory), sizeof(DirectoryStruct));

		numblock = myFileSystem->superBlock.diskSizeInBlocks;
		tam = numblock * BLOCK_SIZE_BYTES;
